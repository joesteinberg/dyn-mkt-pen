...
file.write("\\item Source: World Bank Exporter Dynamics Database and author's calculations. All specifications control for year fixed effects. Robust standard errors in parentheses. $\\S$, $\\ddagger$, and $\\dagger$ denote significance at the 0.1\\%, 1\\%, and 5\\% levels, respectively.")
...
dregs_m = [ols(formula=f,data=agg_by_d_m).fit(cov_type='HC0') for f in formulas]
dregs_p = [ols(formula=f,data=agg_by_d_p).fit(cov_type='HC0') for f in formulas]
...
file.write('\\end{tabular}\n')
file.write('\\begin{tablenotes}\n')
file.write("\\item Source: World Bank Exporter Dynamics Database, CEPII Gravity Database, and author's calculations. All specifications control for year fixed effects. Robust standard errors in parentheses. $\\S$, $\\ddagger$, and $\\dagger$ denote significance at the 0.1\\%, 1\\%, and 5\\% levels, respectively.")
file.write('\\end{tablenotes}\n')
file.write('\\end{threeparttable}\n')
file.write('\\end{center}\n')
file.write('\\normalsize\n')
