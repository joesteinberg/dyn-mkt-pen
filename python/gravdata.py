...
tmp2 = pd.merge(left=tmp1,right=countries,how='left',left_on=['o_country'],right_on=['country'])
tmp2 = tmp2.rename(columns={'code':'iso3_o'})#.drop(['d_country','o_country','country'],axis=1)
...
#merged2 = merged2[pd.notnull(merged2.tariff)]
...
#formula0 = 'np.log(Exports) ~ np.log(1.0+tariff) + np.log(distw)'

controls = ['np.log(gdpcap_d)',
            'np.log(gdpcap_o)',
            'np.log(pop_d)',
            'np.log(pop_o)']
#controls = ['C(iso3_d)','C(iso3_o)']
...
#beta_tariff = reg.params['np.log(1.0 + tariff)']
#beta_dist = reg.params['np.log(distw)']
...
merged2[['iso3_o','iso3_d','year','Exports','pop_d','gdpcap_d','tau']].to_pickle('output/gravdata.pik')
