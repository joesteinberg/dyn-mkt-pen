...
for country in countries:
    ...
    tmp=tmp[['c','y0','y1','f','industry','h','maquiladora','d','y','v']].reset_index(drop=True)
...
# create separate non-maquiladora version of Mexico
#tmp = merged[np.logical_and(merged.c=='MEX',merged.maquiladora==False)].reset_index(drop=True)
#tmp['c'] = 'MEX2'
#merged=merged.append(tmp)

# ensure firm codes are unique
merged['f'] = merged.c + merged.f

# now aggregate by f/d/y
merged = merged.groupby(['c','y0','y1','f','industry','maquiladora','d','y'])['v']\
           .sum()\
           .reset_index()\
...
for k in range(1,TMAX+1):
        merged.loc[np.logical_and(merged.tenure.shift()==k-1,incumbent_mask),'tenure']=k
...
merged=merged[merged.y<merged.y1]
...
# very high exit rate
merged=merged[~( (merged.c=='BGR') & (merged.d=='IRQ'))]
l6=len(merged)
...
#merged.loc[merged.c=='MEX2','c'] = 'MEX'
merged2 = pd.merge(left=merged,right=grav,how='left',on=['c','d','y'])
merged2['v_norm'] = merged2.v/(merged2.popt*merged2.gdppc)
#merged2.loc[merged2.f.str.contains('MEX2'),'c']='MEX2'

##############################################################################################3

#print('Computing number of firms and aggregate trade growth per d/y...')

#tmp = merged2.groupby(['c','d','y'])['f'].\
#      agg([('nf',lambda x:x.nunique())])\
#      .reset_index()

#merged2 = pd.merge(left=merged2,right=tmp,how='left',on=['c','d','y'])
