...
df = pd.read_pickle('output/pik/bra_microdata_processed.pik')
df['c']='BRA'

dfs = pd.read_pickle('output/pik/model_microdata_processed.pik')
dfs['c']='BRA'
...
dregs = [ols(formula=f,data=agg_by_d).fit(cov_type='cluster',cov_kwds={'groups':agg_by_d['d']})]
sregs = [ols(formula=f,data=agg_by_d_s).fit(cov_type='cluster',cov_kwds={'groups':agg_by_d_s['d']})]

sregs_alt = []
if pref!='':
    sregs_alt = [ols(formula=f,data=agg_by_d_s_alt).fit(cov_type='cluster',cov_kwds={'groups':agg_by_d_s_alt['d']})]
...
vars = ['drank']
vnames = ['Avg. rank']
file = open('output/by_nd/tableB2_drank_regs'+pref+'.tex','w')
...
plt.savefig('output/by_nd/figB1_dist_by_nd_model_vs_data_wide.pdf',bbox='tight')
plt.clf()
plt.close()
...
# latex output
file = open('output/by_nd/tableB3_exit_by_nd_drank'+pref+'.tex','w')
...
plt.sca(axes1[0])
plt.savefig('output/by_nd/figB2_by_nd_drank_model_vs_data.pdf',bbox_inches='tight')
