...
//	7. Life cycles
//
//	8. Primary work function that solves, simulates, creates output files, and calls python scripts
//
//      9. Main function
//
...
// sensitivity analysis_flags used to modify output filenames
int calibrate=0;
int sensitivity=0;
int same_alpha_beta_n=0;
int same_alpha_beta_o=0;
int no_alpha=0;
...
int init_params()
{
  ...
  // copied from best Niagara output
  delta0 = 34.65234;
  delta1 = 0.00309521;
  sig_x =  1.02;
  rho_x = 0.98194358;
  ...
  FILE * file = fopen("../python/output/calibration/dests_for_c_program.txt","r");
  if(!file)
    {
...
double entrant_foc(double mp, void * data)
{
  ...
  double plose = exp(-phi1*pow(L[id],om1)*mp);
  double flose = exp(-chi*pow(L[id],zeta)*mp);
  
  int imp = gsl_interp_accel_find(acc, m_grid, NM, mp);
  int imp_lose = gsl_interp_accel_find(acc, m_grid, NM, mp*(1.0-flose));
  ...
  for(int izp=0; izp<NZ; izp++)
      ...
      if(z_trans_probs[iz][izp]>1.0e-11)
	{
	  ...
	  double Vm_lose = 0.0;
	  if(plose>1.0e-8)
	    Vm_lose = interp_with_ix(m_grid,dV[id][ix][izp],NM,mp,imp_lose);

	  Vm = plose*Vm_lose + (1.0-plose)*Vm;
	  ...
	  if(rho_x<0.999)
	    {
	      ...
	      if(plose>1.0e-8)
		Vm2_lose = interp_with_ix(m_grid,EdV_xp[id][izp],
					  NM,mp,imp_lose);

	      Vm2 = plose*Vm2_lose + (1.0-plose)*Vm;
	    }
	  ...
	  for(int t=0; t<NT*2; t++)
	    {
	      ...
	      m_rand[id][i][t] = gsl_rng_uniform(r);
	      if(id==0)
		{
      ...
      for(int kt=0; kt<max_kt; kt++)
	{
	  ...
	  if(kt<max_kt-1)
	      ...
	      else
		{
		  ...
		  // if you got a good or bad marketing shock, update mp
...
int work(const double params[NP], double * error)
{
  ...
  else if(same_alpha_beta_n==1)
    create_panel_dataset("output/abn1_microdata.csv");
  else if(same_alpha_beta_n==2)
    create_panel_dataset("output/abn2_microdata.csv");
  else if(same_alpha_beta_o==1)
    create_panel_dataset("output/abo1_microdata.csv");
  else if(same_alpha_beta_o==2)
    create_panel_dataset("output/abo2_microdata.csv");
  else if(no_alpha==1)
    create_panel_dataset("output/a0_microdata.csv");

  time(&stop);

  if(sensitivity==0 && calibrate==1)
    {
	...
	return 1;
      ...
      if(system("python3 -W ignore ../python/life_cycle_prep.py"))
	return 1;

      if(system("python3 -W ignore ../python/life_cycle_figs.py"))
	return 1;

      printf("\nData processing complete! Time: %0.0f seconds.\n",difftime(stop,start));

      FILE * file = fopen("../python/output/calibration/calibration_data2.txt","r");
      if(!file)
	{
...
int main(int argc, char * argv[])
{
  ...
  else if(argc==2 && strcmp(argv[1],"-alpha-beta-n-1")==0)
    {
      sensitivity=1;
      same_alpha_beta_n=1;
    }
  else if(argc==2 && strcmp(argv[1],"-alpha-beta-n-2")==0)
    {
      sensitivity=1;
      same_alpha_beta_n=2;
    }
  else if(argc==2 && strcmp(argv[1],"-alpha-beta-o-1")==0)
    {
      sensitivity=1;
      same_alpha_beta_o=1;
    }
  else if(argc==2 && strcmp(argv[1],"-alpha-beta-o-2")==0)
    {
      sensitivity=1;
      same_alpha_beta_o=2;
    }
  else if(argc==2 && strcmp(argv[1],"-no-alpha")==0)
    {
      sensitivity=1;
      no_alpha=1;
    }  
  ...
  if(same_alpha_beta_n==1)
    {
      printf("\nAlternative calibration: alpha_n = beta_n\n");
      alpha_n = beta_n;
    }
  else if(same_alpha_beta_n==2)
    {
      printf("\nAlternative calibration: beta_n = alpha_n\n");
      beta_n = alpha_n;
    }  
  else if(same_alpha_beta_o==1)
    {
      printf("\nAlternative calibration: alpha_o = beta_o\n");
      alpha_o = beta_o;
    }
  else if(same_alpha_beta_o==2)
    {
      printf("\nAlternative calibration: beta_o = alpha_o\n");
      beta_o = alpha_o;
    }
  else if(no_alpha==1)
    {
      printf("\nAlternative calibration: alpha_n = alpha_o = 0\n");
      alpha_n = 1.0;
      alpha_o = 1.0;
    }
  ...
  // fine-tune parameters using local optimization
  else if(calibrate==1)
    {
      verbose=1;
      linebreak();
